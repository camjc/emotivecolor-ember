...
var ntc = {
    ...
    init: function () {
        ...
        for (var i = 0; i < ntc.names.length; i++) {
            ...
            rgb = ntc.rgb(color);
            hsl = ntc.hsl(color);
            ntc.names[i].push(rgb[0], rgb[1], rgb[2], hsl[0], hsl[1],
                hsl[2]);
        }
    },
    ...
    name: function (color) {
        ...
        if (color.length < 3 || color.length > 7) {
            return ["#000000", "Invalid Color: " + color, "#000000",
                "", false];
        }
        if (color.length % 3 === 0) {
            color = "#" + color;
        ...
        if (color.length === 4) {
            color = "#" + color.substr(1, 1) + color.substr(1, 1) +
                color.substr(2, 1) + color.substr(2, 1) + color.substr(
                    3, 1) + color.substr(3, 1);
        }
        ...
        for (var i = 0; i < ntc.names.length; i++) {
            if (color === "#" + ntc.names[i][0]) {
                return ["#" + ntc.names[i][0], ntc.names[i][1], ntc.shadergb(
                    ntc.names[i][2]), ntc.names[i][2], true];
            }

            ndf1 = Math.pow(r - ntc.names[i][3], 2) + Math.pow(g -
                ntc.names[i][4], 2) + Math.pow(b - ntc.names[i][5], 2);
            ndf2 = Math.abs(Math.pow(h - ntc.names[i][6], 2)) + Math.pow(
                s - ntc.names[i][7], 2) + Math.abs(Math.pow(l - ntc.names[
                i][8], 2));
            ndf = ndf1 + ndf2 * 2;
            if (df < 0 || df > ndf) {
                df = ndf;
        ...
        return (cl < 0 ? ["#000000", "Invalid Color: " + color,
            "#000000", "", false
        ] : ["#" + ntc.names[cl][0], ntc.names[cl][1], ntc.shadergb(
            ntc.names[cl][2]), ntc.names[cl][2], false]);
    },
    ...
    hslCamJC: function (color) {
        ...
        return [Math.round(h * 360), Math.round(Math.abs(s) * 100),
    ...
    hsl: function (color) {
        return [(ntc.hslCamJC(color)[0] / 360) * 255, (ntc.hslCamJC(
                color)[1] / 100) * 255, (ntc.hslCamJC(color)[2] / 100) *
            255];
        // "use strict";
    ...
    rgb: function (color) {
        "use strict";
        return [parseInt('0x' + color.substring(1, 3), 16), parseInt(
            '0x' + color.substring(3, 5), 16), parseInt('0x' +
